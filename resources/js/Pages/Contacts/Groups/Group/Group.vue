<template>
    <app-content>
        <div class="page-wrapper">
            ...
            <div class="page-body">
                <div class="container-xl">
                    <div class="row row-deck row-cards">
                        <div class="col-12" v-if="contacts.data.length !== 0">
                            <div class="card">
                                ...
                                >
                                    <p class="m-0 text-muted">
                                        <button
                                            ...
                                            @click="toggleDeleteSelectedModal()"
                                        >
                                            Remove Selected &nbsp;
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
            ...
            <CreateModal :contacts="all_contacts" :properties="properties" />
            <DeleteSelectedModal :contacts="contactsToDelete" />
            <UpdateModal
                v-if="
                    contacts.data.length !== 0 && updateModal.contact !== null
                "
                :countries="countries"
...
export default {
    setup() {
        const group = computed(() => usePage().props.value.group);
        const contacts = computed(() => usePage().props.value.contacts);
        const all_contacts = computed(() => usePage().props.value.all);
    ...
    watch: {
        ...
        "searchForm.search": function (val) {
            if (val === "") {
                ...
                axios
                    .get("/contacts/list/group/search-group", {
                        params: { keywords: val },
                    })
    ...
    methods: {
        ...
        toggleEditModal(id) {
            this.updateModal.contact = this.contacts.data.filter(
                (contact) => contact.id == id
            )[0];
        },
        toggleEditResultModal(id) {
            this.updateModal.contact = this.results.filter(
                (contact) => contact.id == id
            )[0];
        },
